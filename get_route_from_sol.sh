...
head -n 1 $filename | cut -d" " -f5 | dos2unix | tr "\n" "}"
printf "\n\n"
...
grep '^p[0-9]* ' $filename | sed 's/p//g' | sort -nk1 | cut -d" " -f2 | dos2unix| xargs -n 4 | sed 's/ //g' | xargs

for((i=1; i<=$round; i++))
do

printf "The input $mask of the round $i is :\n"
grep "^p[0-9]*Rd$i " $filename | sed -e 's/p//g' -e 's/Rd/ /g' | sort -nk1 | cut -d " " -f3 | dos2unix| xargs -n 4 | sed 's/ //g' | xargs
